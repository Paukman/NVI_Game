...
const AccountMapping = memo(() => {
  ...
  const items = useMemo(() => {
    ...
    if (mappingData) {
      data = mappingData.map((q) => ({
        ...
        managed:
          mappedList?.data?.find((m) => m.id === q?.hotelClientAccountId)?.managementStatusId === 100
            ? 'Yes'
            : mappedList?.data?.find((m) => m.id === q?.hotelClientAccountId)?.managementStatusId === 1
            ? 'No'
            : '',
        ...
        smName: mappedList?.data?.find((m) => m.id === q.hotelClientAccountId)?.salesManager?.displayName || '',
      }));
    }
    ...
    setPage(1);
    return newItems;
  }, [filters, hotelClientAccountMappings, mappingData]);
  ...
  const handleAccount = (e, type = null) => {
    ...
    /*ARMappingAccount({
      ...
      params: {
        hotelId,
        sourceAccountName: e?.value === 'null' ? null : mappedList.data?.find((q) => q.id === e?.value)?.accountName,
        hotelClientAccountId: e?.value === 'null' ? null : e?.value,
      },
    });*/
    sethotelClientAccountMappings({
      params: {
    ...
    setHeaders([
      ...
      },
      {
        ...
        onRender: (args) => {
          ...
          const { dataRow } = args;
          return (
            <IfPermitted page='account-mapping' permissionType='remove'>
              <LinkActions
                items={[
                  {
                    clickId: 'remove',
  ...
  const handleCheckbox = (name, value) => {
    if (name === 'all') {
      ...
      setMappingData(mappingData.map((item) => (item.id === name ? { ...item, isChecked: value } : item)));
    }
  };
  ...
  const handleDownloadAs = ({ value }) => {
    if (baseData && Array.isArray(sortData?.items) && sortData?.items?.length) {
      const dataSet = sortData.items.map((item, index) => {
        const { sourceAccountName, hotelName, mtValue } = item;
        return {
      ...
      exportToXLSX(
        ...
        buildDownloadableFilename({
          ...
          reportName: DownloadableReportNames.arMapping,
        }),
  ...
  return (
    <HotelClientAccountProvider>
      <Fragment>
        ...
        {/* <Drawer open={openBulkActionModal} onClose={() => setOpenBulkActionModal(false)} anchor='right'>
          <CheckboxFilters
            filters={mappedList.data}
            onApply={handleCheckboxFilters}
            onCancel={() => setOpenBulkActionModal(false)}
          />
        </Drawer> */}
        {openBulkActionModal && (
          <AccountBulkMapping
        ...
        <ToolBar>
          ...
          <ToolBarItem>
            <ButtonFilter
              variant='tertiary'
              filtersSelected={dFilters.ms === 0 ? 0 : 1}
        ...
        <ToolBar>
          <ToolBarItem>
            <IfPermitted page='account-mapping' permissionType='map'>
              <Button
                //title={getText('generic.editColumns')}
                text='Bulk Action : Map To'
                variant='tertiary'
                onClick={() => {
          ...
          <ToolBarItem toTheRight>
            <IfPermitted page='account-mapping' permissionType='download'>
              <ButtonDownloadAs
          ...
          {!ARMappingLoading &&
            ...
            sortData?.items?.length > 0 && (
              <ARMappingPaginatedDataTable
                ...
                // this is temp solution, pagination should not be cotrolled
                setPageValue={page}
                onChangePage={(a) => {
                  setPage(a);
                }}
...
AccountMapping.displayName = 'AccountMapping';

export { AccountMapping };
