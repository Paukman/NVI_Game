...
import { getCurrencies } from "../BillPaymentProvider/hooks/utils";
...
const useOneTimeForm = (onChange, state, updateExchangeRate) => {
  ...
  useEffect(() => {
    ...
    register({ name: "amountTo" }, { validate: rules.amountTo });
    register({ name: "isDisplayedToAmount" });
    register({ name: "fromCurrency" });
    register({ name: "maxLimitExceeded" });
  ...
  const handleOnAccountChange = async (e, { name, value }) => {
    ...
    if (
      (name === "from" &&
        state.fromAccountsFormatted?.find(
          account => account.value === value && account?.disabled === true
        )) ||
      (name === "to" &&
        state.billPayeesFormatted?.find(
          account => account.value === value && account?.disabled === true
        ))
    ) {
      return NO_CALL;
    ...
    setValue("maxLimitExceeded", false);
    ...
    if (result?.type === FAILED_CALL && result?.error === EXCEEDING_AMOUNT) {
      setValue("maxLimitExceeded", result.limit);
      // we cannot have amountTo as CAD, only validate amount always
      validateFields({ name: "amount", triggerValidation, getValues });
    }
  ...
  const onBlurAmount = async e => {
    ...
    if (result?.type === SUCCESS_CALL) {
      ...
      result?.type === FAILED_CALL &&
      result?.error === EXCEEDING_AMOUNT
    ) {
      setValue("maxLimitExceeded", result.limit);
      validateFields({ name: "amount", triggerValidation, getValues });
    }
