...
describe("Testing validation", () => {
  ...
  it(">> should return true or false", () => {
    ...
    result = validation("numberOfPayments", "something");
    expect(result).toEqual(true);
    result = validation("numberOfPayments", "");
    expect(result).toEqual(false);
...
describe("Testing rules", () => {
  ...
  it(">> should validate properly for amount balanceLimit", () => {
    const getValues = () => {
      return { from: amountValidationGetValuesFrom };
    };
    const state = amountValidationState;
    const errors = rules.amount(getValues, state);
  ...
  it(">> should validate properly for numberOfPayments requiredNoOfPayments", () => {
    let result = rules.numberOfPayments.requiredNoOfPayments("");
    expect(result).toEqual(false);
    result = rules.numberOfPayments.requiredNoOfPayments("10");
    expect(result).toEqual(true);
  });
  it(">> should validate properly for numberOfPayments ensureIsNumber", () => {
    let result = rules.numberOfPayments.ensureIsNumber("not number");
    expect(result).toEqual(false);
    result = rules.numberOfPayments.ensureIsNumber("10");
    expect(result).toEqual(true);
  });
  it(">> should validate properly for numberOfPayments ensureNumberIsInRange", () => {
    let result = rules.numberOfPayments.ensureNumberIsInRange("0");
    expect(result).toEqual(false);
    result = rules.numberOfPayments.ensureNumberIsInRange("1");
    expect(result).toEqual(true);
    result = rules.numberOfPayments.ensureNumberIsInRange("999");
    expect(result).toEqual(true);
    result = rules.numberOfPayments.ensureNumberIsInRange("1000");
    expect(result).toEqual(false);
  });
});
...
describe("Testing validateFields", () => {
  ...
  it(">> should validate numberOfPayments field", async () => {
    let result = "";
    const triggerValidation = jest.fn(name => {
      return new Promise(resolve => {
        result = name;
    ...
    await validateFields({ name: "numberOfPayments", triggerValidation });
    expect(triggerValidation).toBeCalledTimes(1);
    expect(result).toEqual({ name: "numberOfPayments" });
  });
...
describe("Testing triggerSelectedValidation", () => {
  ...
  it(">> should trigger custom validation with endingOptions.numberOfPayments ", async () => {
    const triggerValidation = jest.fn();
    const localState = {
      ...mockState,
