...
import { getEndDateForNumberOfPayments } from "../BillPaymentProvider/hooks/utils";
...
export const rules = {
  ...
  numberOfPayments: {
    requiredNoOfPayments: value => {
      return validation("numberOfPayments", value);
    },
...
export const validateFields = async ({
  ...
  switch (name) {
    ...
    case "numberOfPayments": {
      await triggerValidation({ name: "numberOfPayments" });
      break;
    }
    case "note": {
...
export const triggerSelectedValidation = async (
  ...
  let validNoOf = true;
  if (validationState.endingOption === endingOptions.numberOfPayments) {
    validNoOf = await runValidation({ name: "numberOfPayments" });
  }
  let validEnd = true;
